...
def main():
    #path = "D:\MAC-Work\TrainOnPOCImages\flickr_logos_27_dataset\flickr_logos_27_dataset_cropped_augmented_images\Cocacola\test"
    #dirs = os.listdir(path)    
    #dirs = os.listdir(CROPPED_AUG_IMAGE_DIR)
    ...
    if len(sys.argv) > 1:
      ...
      test_dirs = [
        ...
        #to select random directory
        #test_dir = np.random.choice(test_dirs)
        
        #to select only 1 directory
        test_dir = test_dirs[0] 
        #test_images_fn = [test_image for test_image in os.listdir(test_dir)]
        #test_image_fn = np.random.choice(test_images_fn, 1)[0]
        
        #for selecting single image
        #test_image_fn = test_images_fn[]  
        test_image_fn = os.path.join(test_dir, '178_blue.jpg')
    print("Test image:", test_image_fn)

        #for selecting random image from test folders
